...
const ScrapDetailsForm = ({ category, onBack }) => {
  ...
  const handleAiAnalysis = async () => {
    ...
    try {
      const res = await fetch("https://scrapido-demo-server.onrender.com/api/ai/analyze-comprehensive", {
        method: "POST",
        body: formData,
      });
  ...
  const handlePickupRequest = async (e) => {
    ...
    try {
      const res = await fetch("https://scrapido-demo-server.onrender.com/api/pickup/request", {
        method: "POST",
    ...
    <div className="scrap-form-with-helperhome">
      <div className="scrap-formhome">
        ...
        <form>
          ...
          <label>4. Upload an image for verification(optional)</label>
          <input
            type="file"
            accept="image/*"
